...
class Siteplan_m extends CI_Model {
    ...
    function get_all_regular() {
		...
		FROM tbl_siteplan
		WHERE status = 'Aktif'
		ORDER BY id_cluster asc, nama_siteplan asc");

        return $query;
    }
    
    function get_siteplan($id_unit) {
        $query = $this->db->query("
		SELECT *
		FROM tbl_unit_siteplan
		WHERE id_unit = '" . $id_unit . "' ");

        return $query;
    }

    # Mendapatkan data semua siteplan berdasarkan pilihan

    function get_all_regular_by_name($id_cluster) {
        $query = $this->db->query("
		SELECT *
		FROM tbl_siteplan
		WHERE status = 'Aktif' and  id_cluster = '" . $id_cluster . "'
		");
