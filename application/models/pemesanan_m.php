...
//        }
        ...
        if ($status == "all") {
            $query = $this->db->query("	SELECT 		tbl_pemesanan.id_pemesanan, tbl_virtual_account.no_va,tbl_pemesanan.nomor_pemesanan, tbl_pemesanan.tanggal_pemesanan, tbl_pemesanan.id_agen,
				...
				tbl_pemesanan.status_pemesanan, tbl_pemesanan.status_verify,tbl_pemesanan.status_cetak_spp, CONCAT(tbl_pemesanan.tipe_pembayaran, ' ', tbl_pemesanan.tahap_pembayaran) AS cara_pembayaran, tbl_pemesanan.booking_fee, 
...
//        }
    ...
    function add_va($data) {
        $this->db->insert($this->tbl_va, $data);
        return $this->db->insert_id();
    }

    # Mengecek data resepsionis

    function get_resepsionis($id_customer) {
        $this->db->where('id_customer', $id_customer);
        $query = $this->db->get($this->tbl_resepsionis, 1, 0);
        return $query;
    }

    # Menyimpan data va

    function add_resepsionis($data) {
        $this->db->insert($this->tbl_resepsionis, $data);
        return $this->db->insert_id();
    }

    # Menyimpan data resepsionis

    function edit_resepsionis($id_customer, $data) {
        $this->db->where('id_customer', $id_customer);
    ...
    function delete_by_nup($nomor_pemesanan) {
    ...
    function save_rencana($data) {
        $this->db->insert($this->tbl_rencana, $data);
        return $this->db->insert_id();
    }
    ...
    function get_rencana($id_pemesanan) {
        $query = $this->db->query("	SELECT 
			...
			INNER JOIN 	tbl_user ON tbl_pemesanan.id_user = tbl_user.id_user
			LEFT JOIN 	tbl_virtual_account ON tbl_pemesanan.id_pemesanan = tbl_virtual_account.id_pemesanan
			LEFT JOIN 	tbl_rencana ON tbl_pemesanan.id_pemesanan = tbl_rencana.id_pemesanan
			INNER JOIN 	tbl_customer ON tbl_pemesanan.id_customer = tbl_customer.id_customer
			INNER JOIN 	tbl_unit ON tbl_pemesanan.id_unit = tbl_unit.id_unit
			INNER JOIN 	tbl_cluster ON tbl_unit.id_cluster = tbl_cluster.id_cluster
			LEFT JOIN 	tbl_type ON tbl_unit.id_type = tbl_type.id_type
