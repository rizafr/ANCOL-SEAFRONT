...
# Load menu dashboard
    ...
    <div id="frame_data" style="margin-left: 100px;">
        ...
        <form action="<?php echo base_url(); ?>booking" method="post"  name="tampilCluster" onchange='this.form.submit()'>
            <select name="id_cluster" id="filter_cluster">
                <option value=""> -- Tower -- </option>

                <?php
                $sel = '';
                foreach ($cluster as $data_cluster) {
                    echo '<option value="' . $data_cluster->id_cluster . '" ' . ($data_cluster->id_cluster == $id_cluster ? "selected " : "") . '>' . $data_cluster->nama_cluster . ' (' . $data_cluster->kode_cluster . ')</option>';
                    $sel = '';
                }
                ?>
